...
<table class="m-table" style="margin-bottom:0">
    ...
    <tbody>
        ...
        <tr>
            <td>{{ $stmt->id }}</td>
            <td style="font-size:14px">{{ $stmt->type ?: '—' }}</td>
            <td style="font-size:14px">{{ $stmt->from?->format('d.m.Y') ?? '—' }}</td>
            <td style="font-size:14px">{{ $stmt->to?->format('d.m.Y') ?? '—' }}</td>
            <td style="text-align:right;font-family:monospace;font-size:14px">{{ number_format($stmt->opening_balance, 2, ',', ' ') }} Kč</td>
            <td style="text-align:right;font-family:monospace;font-size:14px">{{ number_format($stmt->closing_balance, 2, ',', ' ') }} Kč</td>
            <td style="font-size:14px">{{ $stmt->user?->name }} {{ $stmt->user?->surname }}</td>
            <td>
