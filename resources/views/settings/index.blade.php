...
<form method="POST" action="{{ route('settings.update-network') }}">
    ...
    <table class="extended" cellspacing="0">
        ...
        <tr>
            <th>DHCP lease time (s)</th>
            <td>
                <input type="number" name="dhcp_lease_time"
                    value="{{ $networkSettings['dhcp_lease_time'] ?? '10800' }}"
                    style="width:100px" min="60" placeholder="10800">
                <small style="color:#888;">Výchozí 10800 = 3 hodiny.</small>
            </td>
        </tr>
        <tr>
            <th>IPv6 prefix</th>
