...
<head>
<meta charset="utf-8">
<title>Podpis dodatku — PVfree.net</title>
...
<h1>Podpis dodatku ke smlouvě</h1>
...
<div id="signedBox" class="m-card" style="display:none">
    ...
    <p>Dodatek byl úspěšně podepsán. Niže si můžete prohlédnout finální PDF.</p>
...
<div id="metaBox" class="m-card" style="display:none">
    ...
    <p class="muted" style="margin-top:10px">
        Tento formulář slouží k elektronickému podpisu dodatku ke smlouvě.
...
(function(){
    ...
    const URLS = {
        ...
        preview:     @json(route('sign.addon.preview')),
        otpSend:     @json(route('sign.addon.otp.send')),
        otpVerify:   @json(route('sign.addon.otp.verify')),
    };
    ...
    async function init() {
        ...
        // Addon already signed → show signed PDF
        if (json.addon_signed) {
            $('signedFrame').src = URLS.preview + '?t=' + encodeURIComponent(TOKEN);
            show($('signedBox'));
            return;
        }

        // Active addon-sign flow
    ...
    async function onConfirm() {
        ...
        try {
            ...
            // Reload preview to show signed PDF
