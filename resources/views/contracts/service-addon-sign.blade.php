...
<head>
<meta charset="utf-8">
<title>Podpis dodatku – dodatečná služba — PVfree.net</title>
...
<h1>Podpis dodatku – dodatečná služba</h1>
...
<div id="metaBox" class="m-card" style="display:none">
    <div class="m-card-title">Údaje o smlouvě a službě</div>
    <div class="meta-grid">
        ...
        <div><div class="lbl">Služba</div><strong id="service_name">—</strong></div>
        <div><div class="lbl">Cena</div><strong id="service_price">—</strong></div>
        <div><div class="lbl">Účinnost od</div><strong id="effective_date">—</strong></div>
    </div>
    <p class="muted" style="margin-top:10px">
        Tento formulář slouží k elektronickému podpisu dodatku o dodatečné službě.
...
(function(){
    ...
    const URLS = {
        info:        @json(route('sign.service_addon.info')),
        preview:     @json(route('sign.service_addon.preview')),
        otpSend:     @json(route('sign.addon.otp.send')),
        otpVerify:   @json(route('sign.service_addon.otp.verify')),
    };
    ...
    async function init() {
        ...
        $('service_name').textContent = json.service_name || '—';
        $('service_price').textContent = (json.service_price != null)
            ? (String(json.service_price).replace(/\.00$/, '') + ' Kč / měsíc') : '—';
