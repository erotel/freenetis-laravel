...
class PdfService
{
    ...
    public function renderServiceAddonPdf(\App\Models\ContractAddon $addon, bool $preview = true, ?Request $request = null): string
    {
        ...
        // Účinnost: přidání = dnem podpisu (akceptace zákazníkem), zrušení =
        // dnem vydání Poskytovatelem (bez podpisu, snižuje závazek).
        $effectiveBasis = $isRemove ? 'vydání' : 'podpisu (akceptace)';

        // Právo na odstoupení do 14 dnů (distanční/elektronický podpis) se týká
        // jen PŘIDÁNÍ služby, které Účastník podepisuje; u zrušení bez podpisu ne.
        $withdrawalSection = $isRemove ? '' : (
            ...
            . '<div class="block"><strong>4.3.</strong> Účastník výslovně žádá, aby poskytování služby bylo zahájeno již ve lhůtě pro odstoupení, tj. ke dni účinnosti dle bodu 1.1. Odstoupením se tento Dodatek od počátku ruší a doplňková služba se neposkytuje.</div>'
        );
