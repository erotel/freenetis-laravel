...
Route::get('ares/lookup-public/{ico}', function (string $ico) {
    ...
    try {
        ...
        // Město/ulici z ARES (důvěryhodný zdroj) najdi, a když chybí, založ.
        $addr = app(\App\Services\AddressResolverService::class)
            ->resolveOrCreate($mestNazev, $psc, $uliceNazev);
        ...
        return response()->json([
            ...
            'town_created'   => $addr['town_created'],
            'street_created' => $addr['street_created'],
        ]);
    } catch (\Exception $e) {
...
Route::prefix('sign')->name('sign.')->group(function () {
    ...
    Route::get ('service-addon',            [\App\Http\Controllers\Contracts\PublicSignController::class, 'showServiceAddon'])->name('service_addon.show');
...
Route::middleware('auth')->group(function () {
    ...
    Route::get ('me/notifications',  [MemberController::class, 'myNotificationsForm'])->name('me.notifications');
    Route::post('me/notifications',  [MemberController::class, 'myNotificationsUpdate'])->name('me.notifications.update');
    ...
    Route::put('allowed-subnets/{id}/billing',        [AllowedSubnetController::class, 'updateBilling'])->name('allowed_subnets.update_billing');
    ...
    // Dodatek: dodatečná služba (fáze A — vytvořit/náhled/smazat; podpis ve fázi B)
    Route::post('members/{id}/contract/service-addon',       [ContractController::class, 'createServiceAddon'])->name('contracts.service_addon.create');
    Route::get('contracts/service-addon/{addonId}/preview',  [ContractController::class, 'previewServiceAddon'])->name('contracts.service_addon.preview');
    Route::delete('contracts/service-addon/{addonId}',       [ContractController::class, 'deleteServiceAddon'])->name('contracts.service_addon.delete');
